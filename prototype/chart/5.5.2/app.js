...
angular.module('particleApp', ['lucidComponents', 'ngDraggable', 'ngSortable', 'dndLists'])
    ...
    .controller('mainCtrl', function($scope, $timeout, $rootScope) {
        ...
        ///////
        $scope.$watch('megamenutipviewed + sharetipviewed + pagestipviewed + betatipviewed', function(newValue) {
            $scope.nextTip();
        });
        $scope.nextTip = function() {
            console.log('next tip');
            if(!$scope.betatipviewed) {
                return $scope.onBoardingTip = 1;
            }
            if (!$scope.pagestipviewed) {
                return $scope.onBoardingTip = 2;
            }
            if (!$scope.megamenutipviewed) {
                return $scope.onBoardingTip = 3;

            }
            if (!$scope.sharetipviewed) {
                return $scope.onBoardingTip = 4;
            }
